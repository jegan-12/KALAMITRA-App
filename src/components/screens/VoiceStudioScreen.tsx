...
export const VoiceStudioScreen: React.FC<VoiceStudioScreenProps> = ({
  ...
  const [seconds, setSeconds] = useState(81); // 01:21
  ...
  const transcriptText = language === 'hi' 
    ...
    <div className="flex flex-col w-full pb-24 pt-2 max-w-md mx-auto">
      ...
      <section className="px-4 pt-2 pb-3">
        <div className="bg-[#f0f5f0] rounded-2xl p-3 flex flex-col gap-2 shadow-sm border border-[#dfe4df]">
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <div className="flex flex-col min-w-0">
                <span className="text-[14px] text-[#181d1a] font-bold truncate">
                  {language === 'hi' ? 'हिन्दी • प्राथमिक' : 'English • Primary'}
                </span>
                <span className="text-[12px] text-[#2c694e] truncate">
                  {language === 'hi' ? 'वाणी पहचान सक्रिय (हिन्दी)' : 'Speech recognition active (English)'}
                </span>
              </div>
            </div>
            ...
            >
              <span className="text-[12px] font-semibold">{language === 'hi' ? 'बदलें' : 'Change'}</span>
          ...
          <div className="flex items-center justify-between px-1 pt-1 border-t border-[#dfe4df]/60">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[12px] text-[#2c694e] font-bold tracking-tight">
                {language === 'hi' ? 'स्पष्ट आवाज़ रिकॉर्ड हो रही है • ९८% सटीकता' : 'Listening clearly • 98% Speech Clarity'}
              </span>
            </div>
        ...
        <div className="relative w-64 h-64 flex items-center justify-center">
          ...
          >
            ...
            <span className="text-[12px] tracking-wider uppercase font-bold mt-1 text-[#ffdbcf]">
              {isPaused ? (language === 'hi' ? 'रुका हुआ' : 'Paused') : (language === 'hi' ? 'रिकॉर्डिंग' : 'Recording')}
            </span>
          </button>
        </div>
        ...
        <div className="flex flex-col items-center text-center mt-2">
          <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-[#e5e9e4] text-[#181d1a] shadow-sm">
            ...
            <span className="text-[12px] text-[#57423b]">
              {language === 'hi' ? 'अपने शिल्प के बारे में सहजता से बताएं' : 'Speak naturally about your craft in English'}
            </span>
          </div>
        </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[12px] uppercase tracking-wider text-[#2c694e] font-bold">
                {language === 'hi' ? 'लाइव प्रतिलेखन • AI अनुवादित' : 'Live Transcription • AI Translated'}
              </span>
            </div>
            <button
              type="button"
        ...
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-1.5">
            ...
            <h2 className="text-[18px] text-[#181d1a] font-bold">
              {language === 'hi' ? 'स्वतः निकाले गए विवरण' : 'Auto-Extracted Details'}
            </h2>
          </div>
          <span className="text-[12px] text-[#2c694e] font-bold bg-[#aeeecb] px-2 py-0.5 rounded-full">
            4 {language === 'hi' ? 'सत्यापित' : 'Verified'}
          </span>
        </div>
        ...
        <div className="bg-[#aeeecb]/40 border border-[#2c694e]/20 rounded-2xl p-3 shadow-sm flex items-center gap-3">
          ...
          <div className="flex flex-col min-w-0 flex-1">
            <span className="text-[11px] text-[#316e52] uppercase tracking-wider font-bold">
              {language === 'hi' ? 'शिल्पकार सलाह' : 'Artisan Tip'}
            </span>
            <p className="text-[13px] text-[#181d1a] font-semibold truncate">
              {language === 'hi' ? 'समय (उदा. २ दिन) और रखरखाव निर्देश जोड़ें' : 'Mention: Time taken (e.g. 2 days) & care instructions'}
            </p>
            <span className="text-[11px] text-[#57423b]">
              {language === 'hi' ? 'खरीदारों को इसे धोने या संभालने का तरीका बताएं' : 'Tell buyers how to wash or maintain this product'}
            </span>
          </div>
        </div>
      </section>
      ...
      <section className="px-4 mt-5 flex flex-col gap-2.5">
        ...
        >
          ...
          <span>{language === 'hi' ? 'AI कैटलॉग तैयार करें' : 'Generate AI Catalog'}</span>
        </button>
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          >
            ...
            <span className="text-[14px] font-bold">
              {isPaused ? (language === 'hi' ? 'जारी रखें' : 'Resume') : (language === 'hi' ? 'रोकें' : 'Pause')}
            </span>
          </button>
          ...
          >
            ...
            <span className="text-[14px] font-bold">
              {language === 'hi' ? 'फिर से बोलें' : 'Restart'}
            </span>
          </button>
        </div>
      </section>
